...
import com.synlabs.ipsaa.jpa.EIDNumberSequenceRepository;
import com.synlabs.ipsaa.jpa.EmployeePaySlipRepository;
import com.synlabs.ipsaa.jpa.EmployeeProfileRepository;
...
import com.synlabs.ipsaa.view.staff.StaffProfileRequest;
import com.synlabs.ipsaa.view.staff.StaffRequest;
import com.synlabs.ipsaa.view.staff.StaffSummaryPageResponse;
...
public class StaffService extends BaseService {
	...
	public StaffSummaryPageResponse list(StaffFilterRequest request) {
		...
		if (request.getPageSize() != null && request.getPageSize() != 0) {
			...
			employees = query.limit(request.getPageSize()).offset(request.getPageSize() * (request.getPageNumber() - 1))
					.fetch();
		} else {
		...
		List<StaffSummaryResponse> list = employees.stream().map(StaffSummaryResponse::new)
	...
	public File getEmployee(StaffFilterRequest staffRequest) {
		...
		if (staffRequest.getEmployerCode().equals("All") || staffRequest.getEmployerCode().equals("ALL"))
			...
			list = employeeRepository.findByActiveTrueAndCostCenterInAndEmployerCode(getUserCenters(),
					staffRequest.getEmployerCode());
		}
	...
	public File getEmployeeSalary(StaffFilterRequest staffRequest) {
		...
		if (staffRequest.getEmployerCode().equals("ALL")) {
			...
			if (employer == null) {
				throw new ValidationException("employer code not found");
			}

			list = employeePaySlipRepository.findByEmployerIdAndMonthAndYear(employer.getId(), staffRequest.getMonth(),
					staffRequest.getYear());
		}

		StaffExcelReport report = new StaffExcelReport(list, staffRequest, exportDirectory,
				staffRequest.getEmployerCode(), employeeSalaryRepository);
	...
	public Employee save(StaffRequest request) throws ParseException {
		...
		if (request.getBiometricId() != null) {
			...
			if (e1 != null) {
				throw new ValidationException(String.format("Duplicate BiometricId [center=%s,eid=%s]",
						e1.getCostCenter().getName(), e1.getEid()));
			}
		}
		...
		if (reportingManager == null) {
			throw new ValidationException(
					String.format("Cannot locate Reporting manager [id = %s]", mask(request.getReportingManagerId())));
		}
		...
		if (center == null) {
			throw new ValidationException(
					String.format("Cannot locate Center[id=%s]", mask(request.getCostCenterId())));
		}
	...
	public Map<String, String> importRecords(MultipartFile file, String dryRun) throws Exception {
		...
		if (!errorInFile && dryRun.equalsIgnoreCase("false")) {
			for (ImportEmployee employee : employees) {
				...
				Employee existingRecord = additionalValidationOnUser(emp.getMobile(), emp.getCostCenter(),
						employee.getEid());
	...
	private EmployeeProfile convertImportEmployeeToEmployeeProfile(ImportEmployee importEmployee) {
		...
		if (!StringUtils.isEmpty(importEmployee.getAddress()) && !StringUtils.isEmpty(importEmployee.getCity())
		...
		if (!StringUtils.isEmpty(importEmployee.getpAddress()) && !StringUtils.isEmpty(importEmployee.getpCity())
	...
	///////////////////// SAMPLE_PATH="C:\\Users\\avnib\\Desktop\\ipsaa\\DesignationUpdate.xlsx";
	//
	// @Transactional
	...
	// entityManager.createQuery("update Employee set designation =:designation
	///////////////////// where eid IN :eids")
