...
public class StaffAttendanceService extends BaseService
{
  ...
  public List<EmployeeAttendance> list()
  {
    ...
    {
      EmployeeAttendance attendance = attendanceRepository.findByEmployeeAndAttendanceDate(employee, LocalDate.now().toDate());
  ...
  public List<EmployeeAttendance> empAttendanceList(EmployeeAttendanceFilterRequest request)
  {
    ...
    List<EmployeeAttendance> attendances = employeeAttendanceRepository.findByEmployeeAndAttendanceDateBetween(employee, from.toDate(), to.toDate());
    return fillAbsent(from.toDate(), to.toDate(), employee, employee.getCostCenter(), attendances, true);
  }

  public List<EmployeeAttendance> fillAbsent(Date from, Date to,
                                             ...
                                             List<EmployeeAttendance> attendances, boolean includeSunday)
  {
    ...
    query.select(holiday).from(holiday)
         .where(holiday.centers.any().eq(center))
         .where(holiday.holidayDate.between(from, to))
         .fetch()
         .forEach(h -> {
  ...
  public void clockin(EmployeeAttendanceRequest request)
  {
    ...
    Employee employee = employeeRepository.findByIdAndCostCenterIn(request.getEmployeeId(), userService.getUserCenters());
  ...
  public File attendanceReport(AttendanceReportRequest request) throws IOException
  {
    ...
    List<EmployeeAttendance> attendances = employeeAttendanceRepository.findByCenterAndAttendanceDateBetween(center, from, to);
    ...
    for (EmployeeAttendance attendance : attendances)
    {
      ...
      row.createCell(4, Cell.CELL_TYPE_STRING).setCellValue(attendance.getEmployee().getExpectedIn() == null ? "" : attendance.getEmployee().getExpectedIn().toString());
      row.createCell(5, Cell.CELL_TYPE_STRING).setCellValue(attendance.getEmployee().getExpectedOut() == null ? "" : attendance.getEmployee().getExpectedOut().toString());

      switch (attendance.getStatus())
      {
        case Present:
          row.createCell(6, Cell.CELL_TYPE_STRING).setCellValue(attendance.getCheckin() == null ? "" : attendance.getCheckin().toString());
          row.createCell(7, Cell.CELL_TYPE_STRING).setCellValue(attendance.getCheckout() == null ? "" : attendance.getCheckout().toString());
  ...
  private List<EmployeeAttendance> fillAbsent(AttendanceReportRequest request,
                                              Center center,
  ...
  public void importAttendanceExcel(MultipartFile file) throws IOException
  {
    ...
    while (xssfRow != null)
    {
      try
      {
        ...
        EmployeeAttendance attendance = employeeAttendanceRepository.findByEmployeeAndAttendanceDate(employee, date);
        if (attendance == null)
        {
          attendance = new EmployeeAttendance();
  ...
  public EmployeeAttendance hrAttendanceSave(HRAdminAttendanceRequest request)
  {
    ...
    if (employee == null)
    {
      throw new ValidationException(String.format("Cannot locate Employee[id=%s]", mask(request.getEmployeeId())));
    }
    EmployeeAttendance attendance = employeeAttendanceRepository.findByEmployeeAndAttendanceDate(employee, request.getDate());
