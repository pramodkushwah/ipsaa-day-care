...
public class DashboardService extends BaseService
{
  ...
  private void calculateFollowUps(DashboardRequest request, StatsResponse response)
  {
    ...
    allQuery.select(event).from(event)
            .where(event.done.isFalse())

            .where(event.callDisposition.in(CallDisposition.Followup,
                                            CallDisposition.Callback,
                                            CallDisposition.NewInquiry,
...
//    TODO : optimize it.
    ...
    openQuery.select(event).from(event)
             .where(event.done.isFalse())
             .where(event.callBack.before(now.toDate()))
             .where(event.callDisposition.in(CallDisposition.Followup,
                                             CallDisposition.Callback,
                                             CallDisposition.NewInquiry,
                                             CallDisposition.Revisit))
    ...
    todayQuery.select(event).from(event)
              .where(event.done.isFalse())
  ...
  private List<Center> getCenters(DashboardRequest request)
  {
    ...
    {
      return usercenters.stream().filter(c -> c.getCode().equals(request.getCenter())).collect(Collectors.toList());
    }
...
//         .where(parent.student.center.in(centers));
    query.select(parent)
         .from(parent)
         .innerJoin(parent.students, student)
         .distinct()
  ...
  private int countCenterCapacity(List<Center> centers)
  {
    ...
    query.select(center.capacity.sum()).from(center)
...
//                              .fetchFirst();
    ...
    BigDecimal costsum = query.select(salary.ctc.sum())
                              .from(salary)
    ...
    getUser().getRoles().forEach(role ->
                                 {
                                   ...
                                   {
                                     List<String> list
                                         = Arrays.stream(role.getDashboard()
                                                             .split(","))
                                                 .map(String::trim)
  ...
  public List<DashStudentFeeResponse> listStudentFee(DashboardRequest request)
  {
    ...
    query.select(studentfee).from(studentfee)
         .where(studentfee.student.active.isTrue())
...
//    List<StudentFeePayment> payments = paymentquery.fetch();
  ...
  public List<FollowUpReportResponse> followupReport(DashboardRequest request)
  {
    ...
    {
      List<InquiryEventLog> logs = eventLogRepository.findByDoneFalseAndInquiryCenterAndCallDispositionIn(center, Arrays.asList(CallDisposition.Followup,
                                                                                                                                CallDisposition.Callback,
                                                                                                                                CallDisposition.NewInquiry,
                                                                                                                                CallDisposition.Revisit));
