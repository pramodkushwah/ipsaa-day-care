...
public class StudentService extends BaseService
{
  ...
  private List<Center> getCenters(StudentFilterRequest request)
  {
    ...
    {
      return usercenters.stream().filter(c -> c.getCode().equals(request.getCenterCode())).collect(Collectors.toList());
    }
  ...
  public PageResponse<StudentSummaryResponse> list(StudentFilterRequest request)
  {
    ...
    if (request.getPageSize() != null && request.getPageSize() != 0)
    {
      ...
      students = query.limit(request.getPageSize())
    ...
    List<StudentSummaryResponse> list = students.stream().map(StudentSummaryResponse::new).collect(Collectors.toList());
  ...
  public StudentResponse saveStudent(StudentRequest request)
  {
    ...
    for (Address address : allAddresses)
    {
      ...
      address.setState(isEmptyOrNa(address.getState()) ?
                       ...
                       address.getState());
      address.setCity(isEmptyOrNa(address.getCity()) ?
                      center.getAddress().getCity() :
                      address.getCity());
      address.setZipcode(isEmptyOrNa(address.getZipcode()) ?
  ...
  public Student updateStudent(StudentRequest request) throws ParseException
  {
    ...
    if (feeChange)
    {
      CenterProgramFee centerFee = centerProgramFeeRepository.findByProgramIdAndCenterId(program.getId(), center.getId());
  ...
  private void updateParents(Student dbStudent, StudentRequest request)
  {
    ...
    for (ParentRequest parentRequest : parents)
    {
      ...
      {
        throw new ValidationException(String.format("Invalid Relationship[%s]", parentRequest.getRelationship()));
      }
      ...
      if (parent != null)
      {
        if (parent.isAccount())
        {
          ...
          {
            throw new ValidationException(String.format("User already exist with email[%s]", parentRequest.getEmail()));
          }
          if (user == null)
          {
  ...
  {
    AdmissionNumberSequence sequence = admissionNumberSequenceRepository.getOneByCenterCode(centerCode).orElse(new AdmissionNumberSequence(centerCode));
...
//    ArrayList<ImportParent> importParentsList = studentRecords.get("parents");
  ...
  private Pair<StudentParent, StudentParent> convertImportParentToStudentParent(Center center, ImportStudentFull importstudent)
  {
...
//    father.setEmergencyContact(importstudent.isEmergencyContact());
  ...
  /**
   * Saves Student Fee in StudentFee entity,
   * throws ValidationException if final fee in request is not matched with calculation in server from baseFee anf discount in request
   * throws NotFoundException if request student not found in database.
   *
   * @param request StudentFeeRequest for saving StudentFee
   ...
   *
   * @param request StudentFeeRequest with id(Masked) to update
   ...
   *
   * @param request filter with center, year, quarter, month and period
   * @return list of fee slips and payments for selected students, year, quarter or month
   */
...
//          break;
  ...
  public List<StudentFeeSlipResponse2> listFeeSlipsTable2(StudentFeeSlipRequest request)
  {
    ...
    List<StudentFeeSlipResponse2> allslips = new LinkedList<>();
    int requestMonth = request.getMonth();
    int requestQuarter = request.getQuarter();
    int requestYear = request.getYear();
...
//          break;
  ...
  /**
   * List fee slips and payments based on the parameters, if the slip is not yet generated then generate the slip
   *
   * @param request filter with center, year, quarter, month and period
   * @return list of fee slips and payments for selected students, year, quarter or month
   */
...
//    1. list students (with fee)
    ...
    {
      case "Monthly":
        //TODO date check to make sure fee slips are only generated += 15 days from start of month
        validateMonth(request);
        for (StudentFee fee : feelist)
        {
          StudentFeePaymentRequest slip = feePaymentRepository.findOneByStudentAndFeeDurationAndMonthAndYear(fee.getStudent(), period, requestMonth, requestYear);
          if (slip == null)
          {
      ...
      case "Quarterly":
        ...
        for (StudentFee fee : feelist)
        {
          StudentFeePaymentRequest slip = feePaymentRepository.findOneByStudentAndFeeDurationAndQuarterAndYear(fee.getStudent(), period, requestQuarter, requestYear);
          if (slip == null)
          {
            BigDecimal baseFee = fee.getFinalFee();
            slip = new StudentFeePaymentRequest();
        ...
        break;
      case "Yearly":
        //TODO date check to make sure fee slips are only generated += 15 days from start of year
        validateYear(request);
        for (StudentFee fee : feelist)
        {
          StudentFeePaymentRequest slip = feePaymentRepository.findOneByStudentAndFeeDurationAndYear(fee.getStudent(), period, requestYear);
          if (slip == null)
          {
            slip = new StudentFeePaymentRequest();
            slip.setStudent(fee.getStudent());
    ...
    switch (feeDuration)
    {
      case Quarterly:
        ...
        else if (dif == 1 || dif == 2)
        {
          ...
          {
            slip.setAutoComments(String.format("BaseFee altered [%s] due to admission in running quarter.", newBaseFee));
          }
        }
        break;
    }
...
//    slip.setReceiptFileName(null);
  ...
  public StudentFeePaymentRequest payFee(SaveFeeSlipRequest request)
  {
   ...
   /* if (request.getExtraCharge() != null)
    ...
    {
      total = total.add(request.getLatePaymentCharge());
    }
    if (request.getDeposit() != null)
    {
      total = total.add(request.getDeposit());
    }
  ...
  {
    SharingSheet sheet = sharingSheetRepository.findByStudentIdAndSharingDate(request.getStudentId(), request.getDate());
  ...
  public SharingSheet createSharingSheetEntry(SharingSheetEntryRequest request)
  {
    ...
    SharingSheet sharingSheet = sharingSheetRepository.findByStudentIdAndSharingDate(request.getStudentId(), request.getDate());
  ...
  public StudentFee getStudentFee(StudentFeeRequest request)
  {
    ...
    else
    {
      ...
      if (student == null)
      {
        throw new ValidationException(String.format("can not locate student[id=%s]", request.getMaskedStudentId()));
      }
  ...
  public List<StudentFeePaymentRequest> listFeeReport(FeeReportRequest request)
  {
    ...
    if(request.getCenterCode().equals("All")){
      List<StudentFeePaymentRequest> slip2 = feePaymentRepository.findByStudentApprovalStatusAndStudentCorporateIsFalseAndFeeDurationAndQuarterAndYear(ApprovalStatus.Approved,period,  request.getQuarter(), request.getYear());
    return slip2;
    }else{
      List<StudentFeePaymentRequest> slip2 = feePaymentRepository.findByStudentApprovalStatusAndStudentCorporateIsFalseAndFeeDurationAndQuarterAndYearAndStudentCenterCode(ApprovalStatus.Approved,period,  request.getQuarter(), request.getYear(),request.getCenterCode());
      return slip2;
    }
  }
  ...
  public List<StudentFeeSlipResponse3> listFeeReport2(FeeReportRequest request)
  {
   ...
   // List<StudentFeePaymentRequest> slip2 = feePaymentRepository.findByStudentActiveIsTrueAndStudentCorporateIsFalseAndFeeDurationAndQuarterAndYearAndStudentCenterCode(period,  request.getQuarter(), request.getYear(),request.getCenterCode());
    // int requestMonth = request.getMonth();
    int requestQuarter = request.getQuarter();
    int requestYear = request.getYear();
    for (StudentFee fee : feelist)
